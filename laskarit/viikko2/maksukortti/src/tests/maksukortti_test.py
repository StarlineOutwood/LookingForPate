...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivinen_lataus_ei_muuta_saldoa(self):
        self.kortti.lataa_rahaa(-800)
        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10.00 euroa")

    def test_voi_syoda_edullisesti_jos_rahaa_vain_edulliseen_lounaaseen(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(str(kortti), "Kortilla on rahaa 0.00 euroa")

    def test_voi_syoda_maukkaasti_jos_rahaa_vain_maukkaaseen_lounaaseen(self):
